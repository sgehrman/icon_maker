...
const float i3=.5773502691896258;

const mat2 tri2cart=mat2(1.,0.,-.5,.5*s3);
...
vec3 pal(in float t){
    ...
    const vec3 a=vec3(.5);
    const vec3 b=vec3(.5);
    const vec3 c=vec3(.8,.8,.5);
    const vec3 d=vec3(0,.2,.5);
...
vec2 randCircleSpline(vec2 p,float t){
    ...
    vec2 pa=randCircle(vec3(p,t1-1.));
    vec2 p0=randCircle(vec3(p,t1));
    vec2 p1=randCircle(vec3(p,t1+1.));
    vec2 pb=randCircle(vec3(p,t1+2.));
    
    vec2 m0=.5*(p1-pa);
    vec2 m1=.5*(pb-p0);
    
    vec2 c3=2.*p0-2.*p1+m0+m1;
    vec2 c2=-3.*p0+3.*p1-2.*m0-m1;
    vec2 c1=m0;
    vec2 c0=p0;
    
    return(((c3*t+c2)*t+c1)*t+c0)*.8;
...
vec2 triPoint(vec2 p){
    float t0=hash12(p);
    return tri2cart*p+.45*randCircleSpline(p,.15*iTime+t0);
}
...
inout vec4 cw){
    ...
    if(d<.5*scl){
        ...
        // 1 just inside
        float w=smoothstep(.5*scl,-.5*scl,d);
...
void mainImage(out vec4 fragColor,in vec2 fragCoord){
    ...
    // get 2D scene coords
    vec2 p=(fragCoord-.5-.5*iResolution.xy)*scl;
    
    // get triangular base coords
    vec2 tfloor=floor(cart2tri*p+.5);
